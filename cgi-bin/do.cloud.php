...
<?php
    ...
    *******************************************************************************/
    ...
    define( "RESTART_MQTT_SCRIPT","/usr/bin/do_mqtt_restart.sh" );
    ...
    **
    */
    function cloud_update( $connection, $writeTables, $readTables, $requestData, &$errors )
    {
        $success = false;

        #error_log("starting cloud_update\n", 3, "cloud_log.txt");
        ...
        else
        {
            ...
            $successfulUpdates = 0;
			$writePtr = $writeTables["CloudSettings"];
			$paramPtr = $readTables["CloudSettings"];
            ...
            @exec( RESTART_MQTT_SCRIPT . "", $output, $returnVal );
    ...
    */
    ...
    #error_log( "Starting do.cloud.php\n", 3, "cloud_log.txt" );
...
//        $readTables = array( "VoIPSettings"=>"VoIPSettings" );

        #error_log( "VenusPerformFormMultiTableUpdate\n", 3, "cloud_log.txt" );
//        error_log( $_POST, 3, "cloud_log.txt");

        if ( !VenusPerformFormMultiTableUpdate( $_POST,
                $writeTables, $readTables, "cloud", "cloud_update", $warnings ) )
        {
