...
<?php
    /******************************************************************************
    **
    **  COMPONENT:          do.glb.php - Venus web-site login support
    **
    **  REVISION:           $Revision:  $
    ...
    **
    */
    function glb_update( $connection, $writeTables, $readTables, $requestData, &$errors )
    {
        $success = false;
        ...
        else
        {
            ...
            foreach( $requestData["params"] as $paramName => $paramValueName )
            {
                ...
                if (VenusValidateParam( $connection, $cleanParamName, $cleanParamValue,
                                        $writeTables["Globals"], $readTables["GlobalsParams"], $errors ))
                {
                    // Skip any selector where there has been no change - indicated by empty string
                    if ( ( $cleanParamName == "ExternalAudioInRoutingMode" ) 
                        || ( $cleanParamName == "TextAndAudioPromptsLanguage" ) )
                    {
    ...
    */
    ...
    if ( isset( $_POST["timestamp"] ) && isset( $_POST[ "sessionID" ] ) )
    {
        ...
        if ( !VenusPerformFormMultiTableUpdate( $_POST,
                $writeTables, $readTables, "glb", "glb_update", $warnings ) )
        {
