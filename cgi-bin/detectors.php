...
<?php
    /******************************************************************************
    **
    **  COMPONENT:          detectors.php - Venus web-site login support
    **
    **  REVISION:           $Revision:  $
    **  LAST MODIFIED BY:   $Author:  $
    **  DATE:               $Date:  $
    **
    **  Presents the detector settings form
    **
    *******************************************************************************
    **
    ...
    */
    ...
    $writeTables = array(  );
    $readTables = array( "Detectors"=>"Detectors",
                         "DetectorsParams"=>"DetectorsParams",
                         "LangDetectors"=>"LangDetectors" );
    ...
    {
        if (VenusIsUpdatableCheck( $con, "detectors", $timeNow, session_id() ) === True  )
        {
            $updateable = True;
        }

        $labelArray = VenusGetLabels( $con,
                $readTables["LangDetectors"], $_SESSION[ SESS_LANGUAGE ] );

        $template = new HTML_Template_ITX( TEMPLATE_FOLDER );
        $template->loadTemplatefile( VenusGetTemplateName( "detectors" ), true, true );

        $template->setCurrentBlock( "DET_FORM" );
        $template->setVariable( "TITLE", $labelArray["PageTitle"] );

        $params = array();
        if ( !($queryResult = @mysqli_query( $con,
                    "select *
                    from {$readTables["Detectors"]} order by ID;" ) ) )
        {
            VenusShowError( $con );
        }
        else
            ...
            switch ( $param["Type"] )
            {
                case DATA_ENTRY_SELECT:
                    $sqlQuery = "select ValueName, Value
                                from {$readTables["DetectorsParams"]}
